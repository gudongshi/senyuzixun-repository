...
// ======================================================================
// 内联组件：OverviewModalComponent（经营总看板弹窗）
// 从 OverviewModal.tsx 内联到此处，避免 Vite Tree Shaking / 模块导入问题
// ======================================================================
...
// ---- 工具函数 ----
function formatOverviewWan(amount: number): string {
...
function formatOverviewPercent(rate: number): string {
...
// ---- 组件 ----
interface OverviewModalComponentProps {
  open?: boolean;
  onClose: () => void;
}

function OverviewModalComponent({ open = false, onClose }: OverviewModalComponentProps) {
  console.log('🔍 OverviewModalComponent 渲染中, open=', open);
  ...
  const fetchOverviewData = useCallback(async () => {
    console.log('🔍 fetchData 被调用, year=', overviewYear, 'businessType=', overviewBusinessType);
    setLoading(true);
    setError(null);
    console.log(`📋 GET /api/stats/overview - 年份: ${overviewYear}, 业务类型: ${overviewBusinessType || '全部'}`);

    try {
      const params = new URLSearchParams();
      params.set('year', String(overviewYear));
      if (overviewBusinessType) params.set('businessType', overviewBusinessType);
  ...
  }, [overviewYear, overviewBusinessType]);
  ...
  useEffect(() => {
    ...
    if (costChartRef.current) {
      ...
      chart.setOption({
        ...
        grid: { left: '3%', right: '5%', bottom: '8%', top: '40px', containLabel: true },
        xAxis: {
          type: 'category',
          data: projectNames,
          axisLabel: {
        ...
        yAxis: [
          {
            ...
            axisLabel: { color: '#94a3b8', formatter: (value: number) => formatOverviewWan(value) },
        ...
        series: [
          {
            name: '实际成本', type: 'bar', data: actualCosts,
            itemStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
          ...
          {
            name: '目标成本', type: 'bar', data: targetCosts,
            itemStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
          ...
          {
            name: '成本进度', type: 'line', yAxisIndex: 1, data: costProgresses,
            lineStyle: { color: '#fbbf24', width: 2.5 },
            itemStyle: { color: '#fbbf24' },
            symbol: 'circle', symbolSize: 6,
            label: { show: true, color: '#fbbf24', fontSize: 10, formatter: '{c}%', position: 'top' },
          },
        ],
      });
      chart.resize();
        ...
        series: [
          {
            ...
            itemStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                { offset: 0, color: '#ef4444' }, { offset: 1, color: '#dc2626' },
              ]),
              borderRadius: [4, 4, 0, 0],
            },
            barMaxWidth: 50,
          ...
          },
          {
            ...
            label: { show: true, color: '#34d399', fontSize: 11, formatter: '{c}%', position: 'top' },
          },
        ],
      });
      chart.resize();
  ...
  if (!open) {
    console.log('🔍 OverviewModalComponent 因 open=false 提前返回 null');
    return null;
  }

  console.log('🔍 OverviewModalComponent open=true，开始渲染弹窗 DOM');
        ...
        <div className="pointer-events-auto w-[90vw] h-[90vh] bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 border border-slate-700/50 rounded-2xl shadow-2xl shadow-blue-900/30 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700/50 bg-slate-900/80 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-white tracking-wide">森宇集团 · 项目经营总看板</h2>
            </div>
            <div className="flex items-center gap-3">
              <select value={overviewYear} onChange={(e) => setOverviewYear(Number(e.target.value))}
              ...
              <select value={overviewBusinessType} onChange={(e) => setOverviewBusinessType(e.target.value)}
                className="bg-slate-800 border border-slate-600 text-slate-200 text-sm rounded-lg px-3 py-1.5 focus:outline-none focus:border-cyan-500/50 transition-colors">
                <option value="">全部业务类型</option>
                {OVERVIEW_BUSINESS_TYPES.map(bt => (<option key={bt} value={bt}>{bt}</option>))}
              </select>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            {error && !loading && (
              <div className="flex flex-col items-center justify-center h-64 gap-3">
                ...
                <button onClick={fetchOverviewData} className="px-4 py-2 rounded-lg bg-cyan-600 text-white hover:bg-cyan-500 transition-colors">重试</button>
              </div>
            )}
            {!loading && !error && data && (
              <>
                <div className="grid grid-cols-4 gap-3">
                  {OVERVIEW_KPI_CARDS.slice(0, 4).map(card => {
                    const value = data.kpi[card.key];
                    const displayValue = card.isPercent ? formatOverviewPercent(value as number) : card.isMoney ? `¥${formatOverviewWan(value as number)}` : String(value);
                    return (
                      <div key={card.key} className={`relative overflow-hidden rounded-xl border ${card.borderColor} bg-slate-800/60 p-4 group hover:scale-[1.02] transition-transform duration-300`}>
                        <div className={`absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r ${card.gradient}`} />
                        <div className="text-2xl font-bold text-white tracking-tight truncate">{displayValue}</div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {OVERVIEW_KPI_CARDS.slice(4).map(card => {
                    const value = data.kpi[card.key];
                    const displayValue = card.isPercent ? formatOverviewPercent(value as number) : card.isMoney ? `¥${formatOverviewWan(value as number)}` : String(value);
                    return (
                      <div key={card.key} className={`relative overflow-hidden rounded-xl border ${card.borderColor} bg-slate-800/60 p-4 group hover:scale-[1.02] transition-transform duration-300`}>
                ...
                <div className="flex gap-4" style={{ minHeight: '380px' }}>
                  <div className="w-44 shrink-0 rounded-xl border border-slate-700/50 bg-slate-800/60 p-3 flex flex-col">
                    <div className="text-xs text-slate-500 font-medium mb-3 px-2">📋 项目导航</div>
                    <nav className="flex-1 flex flex-col gap-0.5">
                      {OVERVIEW_NAV_ITEMS.map(item => (
                        <button key={item.label} onClick={() => toast.info('功能开发中')}
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    <div className="flex-1 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 flex flex-col">
                      <div className="text-sm text-slate-300 font-medium mb-2 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-orange-400" />成本对比分析
                      </div>
                      <div ref={costChartRef} className="flex-1 min-h-0" />
                    </div>
                    <div className="flex-1 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 flex flex-col">
                      <div className="text-sm text-slate-300 font-medium mb-2 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-red-400" />支出分类分析
                      </div>
                ...
                <div className="rounded-xl border border-slate-700/50 bg-slate-800/60 p-4">
                  <div className="grid grid-cols-3 gap-4">
                    <div className="text-center">
                      ...
                      <div className="text-lg font-bold text-cyan-400">¥{formatOverviewWan(data.yearlyNewContract)}</div>
                    </div>
                    <div className="text-center border-x border-slate-700/50">
                      <div className="text-xs text-slate-500 mb-1">本年新签支出合同额</div>
                      <div className="text-lg font-bold text-red-400">¥{formatOverviewWan(data.yearlyNewExpenseContract)}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-slate-500 mb-1">本年新签成本</div>
                      <div className="text-lg font-bold text-amber-400">¥{formatOverviewWan(data.yearlyNewCost)}</div>
                    </div>
                  </div>
                </div>
              </>
            ...
            {!loading && !error && !data && (
              <div className="flex items-center justify-center h-64"><p className="text-slate-500">暂无数据</p></div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between px-6 py-2.5 border-t border-slate-700/50 bg-slate-900/60 shrink-0">
            <div className="flex items-center gap-4 text-xs text-slate-500">
              {data && (<><span>数据更新时间: {new Date(data.lastUpdated).toLocaleString('zh-CN')}</span><span className="text-slate-600">|</span><span>共 {data.totalProjects} 个项目</span></>)}
              {!data && (<span>等待数据加载...</span>)}
            </div>
            <div className="text-xs text-slate-600">森宇集团 · 项目经营总看板</div>
          </div>
        </div>
      </div>
    </>
